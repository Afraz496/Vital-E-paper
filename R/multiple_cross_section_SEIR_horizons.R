...
horizons <- list(c(9,10,14,15,16), c(9,10,14,15,16,21), c(9,10,14,15,16,21,22))
t_shift<-5
...
example_seir_partab <- example_seir_partab %>%
  rows_update(tibble(names='t0', values = 1))

example_seir_partab <- example_seir_partab %>%
  rows_update(tibble(names='t0', fixed = 1))

example_seir_partab <- example_seir_partab %>%
  rows_update(tibble(names='incubation', values = 3))
...
for(horizon in horizons){
  ...
  data <- read_csv(paste0(HOME_WD, data_dir, filename), col_types=cols(Ct.e = "d", Ct.Rdrp = "d"))
  
  dataRAW <- read_csv(paste0(HOME_WD, data_dir, filename), col_types=cols(Ct.e = "d", Ct.Rdrp = "d"))
  ...
  filter_horizon_data(data,t_shift,horizon)
  ...
  res <- foreach(chain_no=1:nchains,.packages = c("virosolver","lazymcmc","extraDistr","tidyverse","patchwork")) %dopar% {
    ...
    output <- run_MCMC(parTab=start_tab,
                       ...
                       PRIOR_FUNC=prior_func_seir,
                       mcmcPars=mcmc_pars,
                       filename=paste0("mcmc_chains/readme_multiple_cross_section/readme_seir_",toString(horizon), "_", chain_no),
...
chains <- load_mcmc_chains(location="final_chains/ProvinceWide",
                           parTab=par_tab,
...
# Apply a case when to update the labels
chains_melted <- chains_melted %>% 
  mutate(names_edited = case_when(
    grepl("I0", name)~ "Proportion Infected at seed time",
    grepl("level_switch", name) ~ "Level Switch",
    grepl("obs_sd", name) ~ "Initial Gumbel Scale Parameter",
    grepl("prob_detect", name) ~ "Prob. Detection",
    grepl("t_switch", name) ~ "Switch Time",
    grepl("viral_peak", name) ~ "Viral Peak",
    grepl("R0", name) ~ "Initial Reproductive Number"))

violin <- ggplot(chains_melted) +
  geom_violin(aes(x=sampno,y=value, fill=factor(horizon_t)), draw_quantiles = c(0.5)) +
  facet_wrap(~names_edited, scales = "free_y")  +
  theme(legend.title=element_blank(), axis.text.x = element_blank(), axis.ticks.x = element_blank()) +
  xlab('') +
  ylab("Value")
violin + scale_fill_manual(values = cbp1)
ggsave(violin, filename = 'Figure4b.png', dpi = 600)
