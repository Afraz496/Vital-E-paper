...
example_seir_partab <- example_seir_partab %>%
  rows_update(tibble(names='I0', values = 1/156))

example_seir_partab <- example_seir_partab %>%
  rows_update(tibble(names='infectious', fixed = 1))

example_seir_partab <- example_seir_partab %>%
  rows_update(tibble(names='I0', upper_bound = 0.06))
...
res <- foreach(chain_no=1:nchains,.packages = c("virosolver","lazymcmc","extraDistr","tidyverse","patchwork")) %dopar% {
  ...
  output <- run_MCMC(parTab=start_tab,
                     ...
                     PRIOR_FUNC=prior_func_seir,
                     mcmcPars=mcmc_pars,
                     filename=paste0("mcmc_chains/readme_multiple_cross_section/readme_seir_",chain_no),
...
chains <- load_mcmc_chains(location="final_chains/Outbreak",
                           parTab=par_tab,
...
chains_melted <- chains_melted %>% 
  mutate(names_edited = case_when(
    grepl("t0", name)~ "Effective Seed Time",
    ...
    grepl("incubation", name) ~ "Incubation"))
violin <- plot_violins_mcmc_params(chains_melted, fill=FALSE, mapping=TRUE)
